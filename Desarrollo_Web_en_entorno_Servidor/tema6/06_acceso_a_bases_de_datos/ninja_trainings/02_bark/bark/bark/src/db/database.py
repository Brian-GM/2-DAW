...
def insert_comment(comment: Comment) -> bool:
    ...
    try:
        with connect(
            host="localhost", user="dwes", password="dwes", database="dwesdb"
        ) as conn:
            with conn.cursor(buffered=True) as cursor:
                     sql = """
                    select * from  into cooments (user, comment, posted, category)
                    values (%s, %s, %s, %s)
                """
                sql = """
                    insert into cooments (user, comment, posted, category)
                    values (%s, %s, %s, %s)
                """
                values = (
                    comment.user,
