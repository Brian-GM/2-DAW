...
def add_sentence(sentence_list: list[dict[str, str | int]]) -> None:
    ...
    """
    add_to_list: dict[str, str | int] = {}
    sentence: str = input("Dime una frase a almacenar: ")
    language: str = input("Dime el idioma de la frase (Ej:Es,En...) ")

    vowels_list: list[str] = ["a", "e", "i", "o", "u"]
    consonants_list: list[str] = ["b", "c", "d", "f", "g", "h", "j", "k", "l", "m", "n", "ñ", "p",
                                  "q", "r", "s", "t", "v", "w", "x", "y", "z"]
    vowels_count: int = 0
    ...
    add_to_list["sentence"] = sentence
    add_to_list["Language"] = language
    add_to_list["vowels"] = vowels_count
    add_to_list["consonants"] = consonants_count
    add_to_list["others"] = others_count

    sentence_list.append(add_to_list)
