...
def insert_comment(comment: Comment) -> bool:
    ...
    try:
        ...
        cursor.execute("SELECT id FROM users WHERE username = %s", (comment.user,))
        user_id = cursor.fetchone()

        # Si el usuario no existe, insertarlo en la tabla users
        if not user_id:
            cursor.execute("INSERT INTO users (username) VALUES (%s)", (comment.user,))
        ...
        cursor.execute(
            "INSERT INTO comments (user, comment, posted, category) VALUES (%s, %s, %s, %s)",
            (user_id, comment.comment, comment.posted, comment.category)
        )
        cursor.connection.commit()
