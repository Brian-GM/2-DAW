...
from .forms import SubjectForm, StudentForm, EnrollmentForm, CompletEnrollmentForm
...
def subject_create(request):
    ...
    return render(
        request, "enrollment/form.html", {"form": form, "header": "Añadir Materia"}
    )
...
def student_create(request):
    ...
    return render(
        request, "enrollment/form.html", {"form": form, "header": "Añadir Alumnado"}
    )


def subject_enroll(request, id):
    subject = get_object_or_404(Subject, id=id)
    message = ""
    if request.method == "POST":
        form = EnrollmentForm(request.POST)
        if form.is_valid():
            try:
                ...
                message = "Ya esta matriculado el alumno"

    else:
        form = EnrollmentForm()
    return render(request, "enrollment/form.html", {"form": form, "message": message})


def complet_enroll(request):
    message = ""
    if request.method == "POST":
        form = CompletEnrollmentForm(request.POST)
        if form.is_valid():
            try:
                Enrollment.objects.create(
